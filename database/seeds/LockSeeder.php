...
class LockSeeder extends Seeder
{
    public function run()
    {
        ...
        $data = Storage::get('core_bans.json');
        ...
        $data = Storage::get('core_bans_active.json');
        $data = json_decode($data);
        ...
        $data = Storage::get('core_bans_if_modifed.json');
        $data = json_decode($data);
        ...
        $data = Storage::get('core_bans_proof.json');
        $data = json_decode($data);

        foreach($data as $proof) {
            if (isset($ids[$proof->parent_ban])) {

                $author = new \SteamID($proof->author);
                $author = $author->ConvertToUInt64();

                $author = User::whereSteamid($author)->first();
                if ($author) {
                    Proofs::create([
                        'lock_id' => $ids[$proof->parent_ban]->id,
                        'path' => $proof->href,
                        'name' => $proof->name,
                        'user_id' => $author->id,
                        'approved' => true,
